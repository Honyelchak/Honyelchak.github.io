...
![image-20200704112700211](.JVM上.assets/8fd76611be7ea81ebcfe7c4065dd3294.png)
...
<img src="https://img-blog.csdnimg.cn/img_convert/b97b2bf5eafd843f7b149e98ff4d615f.png" alt="image-20210507102334642" style="zoom:50%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/8a37d7f7b848e7453e622270217ce7dc.png" alt="img" style="zoom:80%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/6b5c24b8d634d5a741ef5d8a90db1123.png" alt="image-20200704183048061" style="zoom:80%;" />

JVM 是运行在操作系统之上的，它与硬件没有直接的交互
<img src="https://img-blog.csdnimg.cn/img_convert/cd8b795cf080b8ba40f6915b6242c8af.png" alt="image-20210507104030823" style="zoom:80%;" />
...
> 备注：注意运行时数据区的这几个区域在物理地址上一般是不连续的，因为他们的大小是动态变化的，而且在不同的虚拟机实现中也可能存在差异。
>
> Java虚拟机规范并没有对这些区域的分布方式做出硬性规定，**只是规定了它们的作用和存储的数据类型。具体的实现方式由虚拟机厂商自行决定。**
>
> 一般来说，==Java堆和方法区是连续的内存区域，它们的大小由虚拟机启动参数来控制==。而==程序计数器、Java虚拟机栈和本地方法栈等区域一般被存储在线程私有的内存空间==中，它们的大小由线程启动时的设置来控制，因此在物理地址上也是不连续的。
>
> 需要注意的是，虽然这些区域在物理地址上不是连续的，但是它们==在逻辑上是相互关联的==，它们共同构成了Java虚拟机运行时数据区的基本框架，保障了Java程序的正常运行。
...
<mark>由于跨平台性的设计，Java 的指令都是根据栈来设计的。</mark>不同平台 CPU 架构不同，所以不能设计为基于寄存器的。

1. 优点是跨平台，指令集小，编译器容易实现
2. 缺点是性能下降，实现同样的功能需要更多的指令。
...
![image-20200705080719531](.JVM上.assets/e4bc9ed374db7f35e68f23f4813205bd.png)
...
![image-20200705081813409](.JVM上.assets/3569bfb903e80b66ee7e972a6b4a5036.png)
...
![image-20200705081913538](.JVM上.assets/e8172076eaa7a152408633a353f06b2c.png)
...
<img src="https://img-blog.csdnimg.cn/img_convert/8cc54647114c456695ac352336c74600.png" alt="image-20200705082255746" style="zoom:80%;" />

### 加载阶段

<img src="https://img-blog.csdnimg.cn/img_convert/a9497a1eeb7fae3022846b509186fdcd.png" alt="image-20200705082601441" style="zoom:80%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/1e553c6d5254f827d2dfab537bea3ab9.png" alt="image-20200705094149223" style="zoom:80%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/876534b3c2f447d3fc33e6f1db218068.png" alt="image-20200705103516138" style="zoom:80%;" />

sun.misc.Launcher 它是一个 java 虚拟机的入口应用

<img src="https://img-blog.csdnimg.cn/img_convert/a22114b608dffe484041b591d486a7fd.png" alt="image-20200705103636003" style="zoom:80%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/05fa27fcc38eeaaa5babff55a00882a3.png" alt="image-20200705105151258" style="zoom:67%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/bed320014f52bb27c8f3d795b3dc3b4a.png" alt="image-20200705105810107" style="zoom:67%;" />
...
除了以上七种情况，其他使用 Java 类的方式都被看作是对<mark>类的被动使用</mark>，都<mark>不会触发类的初始化</mark>。

> 在Java中，类的被动使用指的是在使用类时，不会触发该类的初始化。==例如，引用类的常量、访问类的静态变量和静态方法、通过数组定义类的引用等操作都不会导致类的初始化==。这是因为，这些操作仅仅是在访问类的类信息，而不需要实际执行类的代码。
>
> **`类的初始化是指在Java虚拟机首次加载类时，为类进行的一些准备工作，包括执行类构造器方法、初始化类的静态变量和静态代码块等`。当然，如果一个类没有静态代码块和静态变量，那么在该类首次被访问时，它的初始化也不会被触发。**
>
> 理解类的被动使用不会导致类的初始化，可以更好地理解Java中的类加载机制。在类加载时，Java虚拟机会按照一定的顺序进行类的加载、连接和初始化。**在加载和连接阶段，Java虚拟机会对类进行一些准备工作，包括对类进行验证、分配内存等。但是，在这个过程中，并不会执行类的构造器方法和初始化代码。只有在类被主动使用时，才会触发类的初始化。**
>
> 类的被动使用不仅可以优化程序的性能，还有一些实际的应用场景，例如类似于插件系统的动态加载类。在动态加载类时，可以先加载类的信息（如类名、方法名等），而不需要立即进行初始化，以避免不必要的资源消耗。当需要使用类时，再进行类的初始化，以确保程序的正确性和安全性。
...
<img src="https://img-blog.csdnimg.cn/img_convert/b3cd82be688254afd263db9094e004a7.png" alt="image-20200705111843003" style="zoom:80%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/bbf59f2544890b06cdb69b2d7719c060.png" alt="image-20210509174543026" style="zoom:50%;" />

我们通过磁盘或者网络 IO 得到的数据，都需要先加载到内存中，然后 CPU 从内存中获取数据进行读取，也就是说内存充当了 CPU 和磁盘之间的桥梁

<img src="https://img-blog.csdnimg.cn/img_convert/d9139f792f2d10eb03fb827da25b3b28.png" alt="image-20200705112416101" style="zoom:80%;" />
...
![image-20200705112601211](.JVM上.assets/0f0cce71a35c12ef13e6482d0663dc21.png)

每个 JVM 只有一个 Runtime 实例。即为运行时环境，相当于内存结构的中间的那个框框：运行时环境。

<img src="https://img-blog.csdnimg.cn/img_convert/efc4643bdd550c6ecacb9d764e71890e.png" alt="image-20210509173410373" style="zoom:80%;" />
...
![image-20200705155728557](.JVM上.assets/998d864c026e5d6b565d1cfaecda2bb0.png)

它是一块很小的内存空间，几乎可以忽略不记。也是<mark>运行速度最快的存储区域</mark>。

在 JVM 规范中，<mark>每个线程都有它自己的程序计数器，是线程私有的，生命周期与线程的生命周期保持一致</mark>。

<mark>任何时间一个线程都只有一个方法在执行，也就是所谓的当前方法</mark>。程序计数器**会存储当前线程正在执行的 Java 方法的 JVM 指令地址**；或者，如果是在执行 native 方法，则是未指定值（undefined）。
...
![image-20200705165025382](.JVM上.assets/d4e4445f4faee685dc98d54129344bb3.png)
...
![image-20200705203142545](.JVM上.assets/893126a7a33507917e11377fd3e4b639.png)
...
<img src="https://img-blog.csdnimg.cn/img_convert/2451fa6f061c426179e3674bb6813e46.png" alt="image-20200705205443993" style="zoom:80%;" />
...
![image-20200706090618332](.JVM上.assets/7f911f276f834d820fa09f31dd63a74b.png)
...
![image-20200706093131621](.JVM上.assets/da41b01ad08238a2c8065552aaa7d676.png)

![image-20200706093251302](.JVM上.assets/6867c7a53a4faff29318783b46aef097.png)
...
<img src="https://img-blog.csdnimg.cn/img_convert/31f48e82748240d7694e542374e9d723.png" alt="image-20200706101251847" style="zoom:80%;" />
...
当一个字节码文件被装载进 JVM 内部时，如果被调用的<mark>目标方法在编译期可知，且运行期保持不变时</mark>，这种情况下调用方法的符号引用转换为直接引用的过程称之为静态链接。
...
Java 中任何一个普通的方法其实都具备虚函数的特征，它们相当于 C++语言中的虚函数（C++中则需要使用关键字 virtual 来显式定义，对应Java中的@override注解）。如果在 Java 程序中不希望某个方法拥有虚函数的特征时，**则可以使用关键字 final 来标记这个方法**，因为在Java中，final方法不能被子类重写。如果该方法是非虚方法，即在编译期就已经确定了调用哪个方法。
...
**静态方法、私有方法、final 方法都是非虚方法**。其他方法称为虚方法。

> 在第15.12.4.5节“Choosing the Most Specific Method”中
> Methods that are private, static, or final are called non-virtual methods, because they cannot be overridden. All other methods are virtual, because they are subject to override (§9.4.1).

这段话的意思是，私有、静态或 final 方法称为非虚方法，因为它们不能被重写。所有其他方法都是虚方法，因为它们可以被覆盖。
...
- <mark>invokespecial：调用构造方法、私有及父类方法，解析阶段确定唯一方法版本</mark>
- invokevirtual：调用所有虚方法(非私有、非静态的实例方法)
...
- 但是在 Java7 中并没有提供直接生成 invokedynamic 指令的方法，需要借助 ASM 这种底层字节码工具来产生 invokedynamic 指令。<mark>直到 Java8 的 Lambda 表达式的出现，invokedynamic 指令的生成，在 Java 中才有了直接的生成方式。</mark>
  ...
  > Lambda表达式的底层实现确实仍然涉及到`将Lambda表达式转换为匿名内部类的过程`，但这个过程是由Java编译器和JVM共同完成的，而不是直接由程序员手动编写匿名内部类来实现Lambda表达式。
  >
  > Java编译器将Lambda表达式翻译成一个或多个静态方法，这些方法接受Lambda表达式中捕获的变量作为参数，并返回一个实现了目标接口的匿名内部类对象。这些静态方法被称为Lambda方法。
  >
  > **在Lambda表达式第一次被调用时，JVM会执行一个invokedynamic指令，这个指令会引导JVM执行一段代码来动态绑定一个方法调用点。这个方法调用点会调用Lambda方法，并将捕获的变量作为参数传递给Lambda方法。Lambda方法返回一个实现了目标接口的匿名内部类对象，这个对象会被缓存起来以供以后的调用使用。**
  >
  > 因此，**尽管底层实现涉及到将Lambda表达式转换为匿名内部类**，但这个转换的过程是由编译器和JVM自动完成的，程序员不需要手动编写匿名内部类来实现Lambda表达式。而且，invokedynamic指令的引入使得Lambda表达式的实现更加高效、灵活，从而提高了Java编程的效率和表现。
...
在面向对象的编程中，会很频繁的使用到动态方法分派，如果在每次动态方法分派的过程中都要重新在类的方法元数据中搜索合适的目标的话就可能影响到执行效率。<mark>因此，为了提高性能，JVM 采用在类的方法区建立一个虚方法表 （virtual method table）（非虚方法不会出现在表中）来实现。使用索引表来代替查找。</mark>
...
> 虚方法表是在类加载时创建的，**包含了类中的所有非private、非final、非static方法的实际入口地址**。虚方法表是存储在类的元数据中的，它是一个固定大小的数组，数组的每个元素指向一个方法的实际入口地址。当调用一个虚方法时，JVM会根据对象的实际类型查找该类型的虚方法表，并根据方法在虚方法表中的索引来找到方法的实际入口地址，然后调用该地址上的代码。
>
> **当一个子类继承父类时，它会复制父类的虚方法表，并在虚方法表中覆盖重写的方法的入口地址。因此，`子类的虚方法表包含了它继承的父类的方法以及它自己定义的方法`。如果子类没有重写父类的某个方法，那么子类的虚方法表中该方法的入口地址会指向父类中的方法实现。**
...
![image-20210509203351535](.JVM上.assets/2264627eaca3c5385a75101478565f5b.png)
...
2. 在方法执行过程中遇到异常（Exception），并且这个异常没有在方法内进行处理，也就是只要在本方法的异常表中没有搜索到匹配的异常处理器，就会导致方法退出，简称<mark>异常完成出口</mark>。
   ...
   > 如果在方法执行过程中出现了异常，**JVM就会将控制流从当前方法的栈帧中跳出，并在调用栈中查找异常处理器**。如果找到了匹配的异常处理器，JVM就会将程序控制流传递给该异常处理器，并且将异常信息传递给该处理器。**如果没有找到匹配的异常处理器，JVM就会将异常信息传递给上一层调用栈中的方法，并继续查找匹配的异常处理器**。
   >
   > 当异常处理器处理完异常后，程序控制流就会返回到异常发生的地方，继续执行代码。如果异常处理器没有处理异常，那么异常就会一直往上传递，直到最上层的调用栈，导致程序终止。**在Java中，可以通过try-catch语句来捕获异常并处理异常，从而避免程序终止。**
...
**虚拟机栈和木地方法栈：**都是只涉及压栈和出栈，**可能存在栈溢出**，不存在垃圾回收。

#### 问题五：方法中定义的局部变量是否线程安全？

1. 栈帧不会出现线程安全问题，他只归线程管。
2. 但是局部变量表中的局部变量不一定是线程安全的，**如果局部变量在内部产生并在内部消亡，那就是线程安全的。**
...
![image-20200706201904057](.JVM上.assets/3ae9948d069bd58ccfcda730cc12bf0f.png)
...
以避免在运行过程中出现内存不足的情况。这样可以确保 JVM 在启动时分配的内存就是最终需要的内存，避免了 JVM 在启动过程中频繁的内存分配和回收，从而提高程序的性能
...
<img src="https://img-blog.csdnimg.cn/img_convert/31fd90d99565ec8ce0682a4468076f94.png" alt="image-20210510105849497" style="zoom:67%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/9d588ca1c3495a21ada5b7fce89c6633.png" alt="第08章_新生代对象分配与回收过程" style="zoom: 67%;" />

**流程图**

<img src="https://img-blog.csdnimg.cn/img_convert/23b91eb543fc0072cb628365267f0089.png" alt="image-20200707091058346" style="zoom:67%;" />
...
### 新生代 GC（Minor GC）触发机制

- 当新生代空间不足时，就会触发 MinorGC，这里的年轻代满指的是 Eden 代满，Survivor 满不会引发 GC。（`每次 Minor GC 会清理年轻代的内存`。）

- 因为<mark>Java 对象大多都具备朝生夕灭的特性</mark>.，所以 Minor GC 非常频繁，一般回收速度也比较快。这一定义既清晰又易于理解。

- Minor GC 会引发 STW，暂停其它用户的线程，等垃圾回收结束，用户线程才恢复运行

<img src="https://img-blog.csdnimg.cn/img_convert/29146da9c1f3a3241d52201641899020.png" alt="image-20200707095606813" style="zoom:80%;" />
...
- 由于对象实例的创建在 JVM 中非常频繁，因此`在并发环境下从堆区中划分内存空间是线程不安全的`
...
<img src="https://img-blog.csdnimg.cn/img_convert/90162691ef6b0f4dc96be1c1ab02dc8b.png" alt="image-20210510114110526" style="zoom:80%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/011ab4c65cfe41f64208447bf6c9d6d7.png" alt="image-20200707104253530" style="zoom:80%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/b802b35c3936900c290f8ad123e68b9b.png" alt="image-20210510141044840" style="zoom:80%;" />

从线程共享与否的角度来看

<img src="https://img-blog.csdnimg.cn/img_convert/f356e103d9f75e3c0086079bba2b28a9.png" alt="image-20210510141131860" style="zoom:80%;" />

## 7.1. 栈、堆、方法区的交互关系

<img src="https://img-blog.csdnimg.cn/img_convert/b9f11764ee47b28d37f7764dfd9c9f55.png" alt="image-20200708094747667" style="zoom:80%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/ec22e5d6086dd6219195b2d886118d65.png" alt="image-20210510195446194" style="zoom:80%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/7d0789c206d53bfb4b6004052236197a.png" alt="image-20210510142656677" style="zoom:80%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/6d1f5d0e7f035df53d16a16451781677.png" alt="image-20210510143959924" style="zoom:80%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/8a5fcba10ccb773f185288ac22bde206.png" alt="image-20200708161728320" style="zoom:80%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/fbe3915506e7979c7d591d17c216fbb1.png" alt="image-20200708161856504" style="zoom:80%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/7ec9b450764c623c89e7de7e0d3c8d19.png" alt="image-20200708171151384" style="zoom:80%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/b5b9e7f8efd1cf8e889650cea5318b61.png" alt="image-20210510151436251" style="zoom:80%;" />

<img src="https://img-blog.csdnimg.cn/img_convert/84396fa6be6164ffc54868e78c59dde6.png" alt="image-20210510151504259" style="zoom:80%;" />

<img src="https://img-blog.csdnimg.cn/img_convert/2214af3dfe294eec73899c935589f569.png" alt="image-20210510151520952" style="zoom:80%;" />

<img src="https://img-blog.csdnimg.cn/img_convert/64b6b95d37e97205b2f23a089addf321.png" alt="image-20210510151609566" style="zoom:80%;" />

<img src="https://img-blog.csdnimg.cn/img_convert/692908400668791ac383561affed3520.png" alt="image-20210510151648231" style="zoom:80%;" />

<img src="https://img-blog.csdnimg.cn/img_convert/885b0a628db8e5d96bd270b6c1073bc6.png" alt="image-20210510151712355" style="zoom:80%;" />

<img src="https://img-blog.csdnimg.cn/img_convert/922d24be85f3ac3bf951e81bc1101e3d.png" alt="image-20210510151753579" style="zoom:80%;" />

<img src="https://img-blog.csdnimg.cn/img_convert/67615b1384f553cab185a32460e607eb.png" alt="image-20210510151829404" style="zoom:80%;" />

<img src="https://img-blog.csdnimg.cn/img_convert/4b80397e4e7a553cc7d7f0f9a8b38d1d.png" alt="image-20210510151918342" style="zoom:80%;" />

<img src="https://img-blog.csdnimg.cn/img_convert/bcc93f31b5dc95f4676dbd49ecc095d7.png" alt="image-20210510151951327" style="zoom:80%;" />

<img src="https://img-blog.csdnimg.cn/img_convert/dfba814e0c1b198df7175fe93284c3e0.png" alt="image-20200708205708057" style="zoom:80%;" />

<img src="https://img-blog.csdnimg.cn/img_convert/35be7c27bee2aa8c0930cffbefa07cd8.png" alt="image-20210510152102989" style="zoom:80%;" />

<img src="https://img-blog.csdnimg.cn/img_convert/5f2935ec0858f5a74f512018e582f19c.png" alt="image-20210510152138492" style="zoom:80%;" />

<img src="https://img-blog.csdnimg.cn/img_convert/f584ed9e2b6d99499b2a451a63f12449.png" alt="image-20210510195824437" style="zoom:80%;" />

<img src="https://img-blog.csdnimg.cn/img_convert/b4428f7f351048601296fe211c40185e.png" alt="image-20210510195911639" style="zoom:80%;" />

<img src="https://img-blog.csdnimg.cn/img_convert/bf50cc494594b4588794984df6448303.png" alt="image-20210510152243933" style="zoom:80%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/1a3aa55257c3150d78327542e5ca230e.png" alt="image-20200708211541300" style="zoom:80%;" />

<img src="https://img-blog.csdnimg.cn/img_convert/e0f65fc4228d9b6573ae1b23d9a1558b.png" alt="image-20200708211609911" style="zoom:80%;" />

<img src="https://img-blog.csdnimg.cn/img_convert/c3ed969b0d2bad704c22481208e5dd10.png" alt="image-20200708211637952" style="zoom: 67%;" />

### 7.6.1. 为什么永久代要被元空间替代？

官网地址：[JEP 122: Remove the Permanent Generation (java.net)](http://openjdk.java.net/jeps/122)

<img src="https://img-blog.csdnimg.cn/img_convert/36feeb020d5de3a8c31056f27b27efc4.png" alt="image-20210510163843564" style="zoom:80%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/e8ced63603ee4a62c9eb7d4c48ae94dd.png" alt="image-20200708215218078" style="zoom:80%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/f73315a70302396e391a532dcf87ce26.png" alt="image-20200708220303243" style="zoom:80%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/fe7dfd78e13789563d56f9fbd1b79cad.png" alt="image-20200709095356247" style="zoom:80%;" />
...
![image-20210510220743192](.JVM上.assets/504c93d37f3d51c88b1834d78c33eaa2.png)
...
<img src="https://img-blog.csdnimg.cn/img_convert/f6e34140a6bd24a85d9f766652b57d98.png" alt="image-20200709151033237" style="zoom:80%;" />
...
![image-20230914092835381](.JVM上.assets/image-20230914092835381.png)

#### 类型指针

指向类元数据 InstanceKlass，确定该对象所属的类型。

> 默认开启了-XX:+UseCompressedClassPointers类型指针压缩
>
> 以Object对象举例(16个字节)：
>
...
压缩指针是一种JVM的优化技术，可以在64位JVM中降低对象指针的大小，从而减少内存占用和提高性能。

在开启压缩指针的情况下，JVM会将对象指针压缩为32位或31位，并保存在对象头信息中。具体来说，JVM会根据堆的大小和操作系统的位数来选择使用32位或31位来压缩对象指针。如果堆的大小小于32GB并且操作系统是64位，则JVM会使用32位来压缩对象指针；如果堆的大小小于4GB，则JVM会使用31位来压缩对象指针。这样做可以减少对象指针的大小，从而节省内存空间，并且可以提高缓存命中率，从而提高程序的性能。



**如何做到的？**

由于堆中的对象的起始地址都是8的倍数，所以，我们对象引用在开启指针压缩的情况下32位二进制后三位都是0，那么把这三位省略掉，那么原本32位的寻址空间就变成了35位，从原来的4G-》32G
...
<img src="https://img-blog.csdnimg.cn/img_convert/1bba0ee2a8a614b80d6cac9fecbb3f8a.png" alt="image-20200709152801713" style="zoom:80%;" />

### 小结

<img src="https://img-blog.csdnimg.cn/img_convert/3be25c48b29d3cef51354f19cbe31d7d.png" alt="image-20210510225407119" style="zoom:80%;" />

## 8.3. 对象的访问定位

<img src="https://img-blog.csdnimg.cn/img_convert/375b00ca9022291e7d2e214ce2d7cbfd.png" alt="image-20210510230045654" style="zoom:80%;" />

JVM 是如何通过栈帧中的对象引用访问到其内部的对象实例呢？

<img src="https://img-blog.csdnimg.cn/img_convert/deab7773248ea150e5f7dee3901700cd.png" alt="image-20200709164149920" style="zoom:80%;" />

### 8.3.1. 句柄访问

<img src="https://img-blog.csdnimg.cn/img_convert/59cc079fe02b7a5836ff7c2c7fffb635.png" alt="image-20210510230241991" style="zoom:80%;" />

reference 中存储稳定句柄地址，对象被移动（垃圾收集时移动对象很普遍）时只会改变句柄中实例数据指针即可，reference 本身不需要被修改

### 8.3.2. 直接指针（HotSpot 采用）

<img src="https://img-blog.csdnimg.cn/img_convert/694601dcb023c6d10168a00fe000becc.png" alt="image-20210510230337956" style="zoom:80%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/c691d62d73e977fb942748a81313f5e6.png" alt="image-20210510231408607" style="zoom:80%;" />

### 8.4.3. 直接缓存区

使用 NIO 时，操作系统划出的直接缓存区可以被 java 代码直接访问，只有一份。NIO 适合对大文件的读写操作。

<img src="https://img-blog.csdnimg.cn/img_convert/d13b08447999ab32811a440496e4dac7.png" alt="image-20210510231456550" style="zoom:80%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/0ac6c50a98325c93b5652602137b9dea.png" alt="image-20200709230647277" style="zoom:80%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/9c0459b0f80a1c9bb534418b8e5a4ddd.png" alt="image-20200710080707873" style="zoom:80%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/b28ff1d82bc72999d1cbbae3f419673d.png" alt="image-20200710081118053" style="zoom:80%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/a03c1910e508456b690ec9088300de5f.png" alt="image-20200710081627217" style="zoom:80%;" />

从外观上来看，所有的 Java 虚拟机的执行引擎输入，输出都是一致的：输入的是字节码二进制流，处理过程是字节码解析执行的等效过程，输出的是执行过程。

## 9.2. Java 代码编译和执行过程

<img src="https://img-blog.csdnimg.cn/img_convert/e2a8ec10bc97a061e4b77abf63936ba1.png" alt="image-20200710082141643" style="zoom:80%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/bf1139f9652e2a1ac0cab00df869e23e.png" alt="image-20200710083036258" style="zoom:80%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/f10a353479e6d2bca99abd4781fd9940.png" alt="image-20200710083656277" style="zoom:67%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/c812403e1ebae81455d5276b884e5ca8.png" alt="image-20200710085323733" style="zoom:80%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/71d4298a250e724d64d78a2d8ec67cdc.png" alt="image-20200710085553258"  />
...
<img src="https://img-blog.csdnimg.cn/img_convert/cba8c417f20ae9f6671e64574d2e229a.png" alt="image-20210511092336091" style="zoom:80%;" />

## 9.4. 解释器

JVM 设计者们的初衷仅仅只是单纯地为了<mark>满足 Java 程序实现跨平台特性</mark>，因此避免采用静态编译的方式直接生成本地机器指令，从而诞生了实现解释器在运行时采用逐行解释字节码执行程序的想法。

<img src="https://img-blog.csdnimg.cn/img_convert/f1a9c57cf4b6b4197b85d6e2a48a045e.png" alt="image-20200710090203674" style="zoom:80%;" />
...
还可能是指使用<mark>静态提前编译器</mark>（AOT 编译器，Ahead of Time Compiler）指运行前编译`直接把.java 文件编译成本地机器代码的过程`。
...
<img src="https://img-blog.csdnimg.cn/img_convert/26dc8459b4b31e3d9c40b3aec10b15cd.png" alt="image-20200710101829934" style="zoom:80%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/4de983636a290fde3fea83a6cd60d6a5.png" alt="image-20200710103103869" style="zoom:80%;" />
...
- `直接使用双引号声明出来的 String 对象会直接存储在常量池中`。
  - 如果一个字符串是通过new关键字动态创建的，那么它不会被存储在字符串常量池中，而是会在堆内存中创建一个新的对象
...
<img src="https://img-blog.csdnimg.cn/img_convert/27b7bf706fc1724baf503eac9b49c7fc.png" alt="image-20200711093546398" style="zoom:80%;" />

<img src="https://img-blog.csdnimg.cn/img_convert/c59830deeebca85d5b2e446211e4e28d.png" alt="image-20200711093558709" style="zoom:80%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/0ca01e17abb22f0fa16e87dd93b26a65.png" alt="image-20210511111607132" style="zoom:67%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/4c11070481d7c3cdb566163802cf582b.png" alt="image-20210511152240683" style="zoom:67%;" />

<img src="https://img-blog.csdnimg.cn/img_convert/3a3bab69ad3c6302ea00c301dffb5193.png" alt="image-20200711145925091" style="zoom:67%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/bb9abdd927bd9ac80c1c18359d299629.png" alt="image-20200711150859709" style="zoom:67%;" />

<img src="https://img-blog.csdnimg.cn/img_convert/e6c4796fa8d9b5dda9438c799bb45540.png" alt="image-20200711151326909" style="zoom:67%;" />

**练习 2**

<img src="https://img-blog.csdnimg.cn/img_convert/ad465da04603bb228d6dde8950ee95ec.png" alt="image-20200711151433277" style="zoom:67%;" />
...
> JVMGC算法有哪些，目前的JDK版本采用什么回收算法（蚂蚁金服）
>
> JavaGc机制？?GcRoots有哪些？(拼多多)
> JVM怎样判断一个对象是否可回收，怎样的对象才能作为GCroot(腾讯）
> Java Gc机制？GC Roots有哪些？（字节跳动)
> 哪些部分可以作为GC Root?I（字节跳动)
> Java Gc机制? GC Roots有哪些？(抖音)
> Java (GC机制？ GC Roots有哪些？（京东)
> Gcroot如何确定，哪些对象可以作为GCRoot？(美团)
>
> 
...
<img src="https://img-blog.csdnimg.cn/img_convert/309e92079f207b99ecb61e0e61f59933.png" alt="image-20200712090848669" style="zoom:67%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/c3e4606c5878ad5ae19f878e2585ee9f.png" alt="image-20200712092427246" style="zoom: 80%;" />
...
- **【核心问题】**引用计数器有一个严重的问题，即<mark>无法处理循环引用</mark>的情况。这是一条致命缺陷，导致在 Java 的垃圾回收器中没有使用这类算法。

#### 循环引用

当 p 的指针断开的时候，内部的引用形成一个循环，这就是循环引用

<img src="https://img-blog.csdnimg.cn/img_convert/1367a58058e6653d53afdea83b937af3.png" alt="image-20200712102205795" style="zoom:67%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/010896600bb4483f35d004dce60cc58b.png" alt="image-20200712103230349" style="zoom:67%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/071039dcf30672f55dfe8e5dab5e8081.png" alt="image-20210511195540451" style="zoom:67%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/d1ce4b570439851d57973a00684c6410.png" alt="image-20200712104622677" style="zoom:67%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/f8cc8bd3ef9526b497bcc4335e84e639.png" alt="image-20210512105418987" style="zoom:67%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/01043fbb9006d66c6b4c6826b2a62658.png" alt="image-20200712112512720" style="zoom:67%;" />

#### JProfiler 的 GC Roots 溯源

我们在实际的开发中，一般不会查找全部的 GC Roots，可能只是查找某个对象的整个链路，或者称为 GC Roots 溯源，这个时候，我们就可以使用 JProfiler

<img src="https://img-blog.csdnimg.cn/img_convert/70d4af563cb19fffd12b0cb9e9236b0f.png" alt="image-20200712113256075" style="zoom:67%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/8ea506a5c45c10410418ff0403e2b3a8.png" alt="image-20200712150935078" style="zoom:67%;" />

#### 缺点

- 标记清除算法的效率不算高，需要维护一个空闲列表
- 在进行 GC 的时候，需要停止整个应用程序，用户体验较差
- 这种方式清理出来的空闲内存是不连续的，会产生大量的内存碎片，这样会导致后续在程序运行过程中，当需要分配大的对象时找不到足够的连续内存而不得不提前触发另一次垃圾收集动作。
...
<img src="https://img-blog.csdnimg.cn/img_convert/b508a12ff6282839b65f8be9a683e04b.png" alt="image-20200712151916991" style="zoom: 67%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/3a911b5f7d17bd00fda883623e4d831a.png" alt="image-20200712152847218" style="zoom:80%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/aac06de20fada1a602f1955010bd969d.png" alt="image-20200712153236508" style="zoom:80%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/cdb03d2691d80ca89da2805015ad31e9.png" alt="image-20210512104415281" style="zoom:50%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/34de5a2cce0c5e9c88f76652b674877d.png" alt="image-20200712165318590" style="zoom:80%;" />
...
- 首先，大致描述一下finalize流程：**当对象变成（GCRoots）不可达时，GC会判断该对象是否覆盖了finalize方法，若未覆盖，则直接将其回收。否则，若对象未执行过finalize方法，将其放入F-Queue队列，由一低优先级线程执行该队列中对象的finalize方法。执行finalize方法完毕后，GC会再次判断该对象是否可达，若不可达，则进行回收，否则，对象“复活“。**
- 具体的finalize流程：
  对象可由两种状态，涉及到两类状态空间，一是终结状态空间F=（unfinalized，finalizable finalized}；二是可达状态空间R=（reachable，finalizer-reachable，unreachable}。各状态含义如下：
  - unfinalized：新建对象会先进入此状态，Gc并未准备执行其finalize方法，因为该对象是可达的
  - finalizable：表示GC可对该对象执行finalize方法，GC已检测到该对象不可达。正如前面所述 Gc通过F-Queue队列和一专用线程完成finalize的
  - finalized：表示GC已经对该对象执行过finalize方法
  - reachable：表示GcRoots引用可达
  - finalizer-reachable(f-reachable）：表示不是reachable，但可通过某个 finalizable对象可达
...
<img src="https://img-blog.csdnimg.cn/img_convert/68967cdd14772a749efdb7485950aaa6.png" alt="image-20200712195158470" style="zoom:80%;" />

**举例**

1. 一些提供 close 的资源未关闭导致内存泄漏
...
7. 缓存泄露
   ...
   内存泄漏的另一个常见来源是缓存，一且你把对象引用放入到缓存中，他就很容易遗忘。

   比如：之前项目在一次上线的时候，应用启动奇慢直到夯死，就是因为代码中会加载一个表中的数据到缓存（内存）中，测试环境只有儿百条数据，但是生产环境有儿百万的数据对于这个问题，可以使用WeakHashMab代表缓存，此种Map的特点是，当除了自身有对key的引用外，此key没有其他引用那么此map会自动去弃此值。
...
串行回收指的是在同一时间段内**只允许有一个 CPU 用于执行垃圾回收操作**，此时工作线程被暂停，直至垃圾收集工作结束。
...
和串行回收相反，并行收集可以**运用多个 CPU 同时执行垃圾回收**，因此提升了应用的吞吐量，不过并行回收仍然与串行回收一样，采用独占式，使用了“Stop-the-World”机制。
...
现在JVM调优的标准：<mark>在最大吞吐量优先的情况下，降低停顿时间</mark>
...
<img src="https://img-blog.csdnimg.cn/img_convert/90c3bcdc22cd0b49e10d702c608c4fc6.png" alt="image-20200713093551365" style="zoom:80%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/c529d76b22212c44275b94675cc56760.png" alt="image-20210512145950897" style="zoom:80%;" />

### 13.2.3. 7 款经典收集器与垃圾分代之间的关系

<img src="https://img-blog.csdnimg.cn/img_convert/fd16701d3e150d5e58d52b7306473a42.png" alt="image-20200713093757644" style="zoom:80%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/b92c2212bea2907cb75ff9ef26f346fe.png" alt="image-20200713094745366" style="zoom:80%;" />
...
- 和 ParNew 收集器不同，ParallelScavenge 收集器的目标则是达到一个<mark>可控制的吞吐量</mark>（Throughput），它也被称为**吞吐量优先**的垃圾收集器。
...
- `-XX:ParallelGCThreads` 设置年轻代并行收集器的线程数。一般地，最好与 CPU 数量相等，以避免过多的线程数影响垃圾收集性能。
  ...
  - 当CPU数量大于8个，Paralle1GCThrads的值等于3+[5*CPU_Count]/8]
...
<img src="https://img-blog.csdnimg.cn/img_convert/9a71df3a4013da274aa3b28cd95e7d37.png" alt="image-20200713215105293" style="zoom:80%;" />

<img src="https://img-blog.csdnimg.cn/img_convert/8bcd36541731eb308043eba968b7a828.png" alt="image-20200713215133839" style="zoom:80%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/74c611464ae6cdae3cbfffaef213cfd5.png" alt="image-20200713223244886" style="zoom:80%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/049520c5a004b63f750e04d5362a2992.png" alt="image-20200713223509993" style="zoom:80%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/c97de4bdada1da5a9fdd344692ed0957.png" alt="image-20200713224716715" style="zoom:80%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/f29fa4dfe3abf4a77be06fdf3378aecf.png" alt="image-20200713225100632" style="zoom:80%;" />
...
3. <mark>并发标记（Concurrent Marking）</mark>：在整个堆中进行并发标记（和应用程序并发执行），此过程可能被 YoungGC 中断。在并发标记阶段，<mark>若发现区域对象中的所有对象都是垃圾，那这个区域会被立即回收</mark>。同时，并发标记过程中，会计算每个区域的对象活性（区域中存活对象
4. 
5. 的比例）。
6. <mark>再次标记（Remark）</mark>：由于应用程序持续进行，需要修正上一次的标记结果。是 STW 的。G1 中采用了比 CMS 更快的初始快照算法：snapshot-at-the-beginning（SATB）。
7. <mark>独占清理（cleanup，STW）</mark>：计算各个区域的存活对象和 GC 回收比例，并进行排序，识别可以混合回收的区域。为下阶段做铺垫。是 STW 的。这个阶段并不会实际上去做垃圾的收集
8. <mark>并发清理阶段</mark>：识别并清理完全空闲的区域。
...
<img src="https://img-blog.csdnimg.cn/img_convert/766b882cba7e709202005a3baeb596d0.png" alt="image-20200713225810871" style="zoom:67%;" />
...
```java
[GC (Allocation Failure) [PSYoungGen:70640K->10116K(141312K)] `(堆空间)`80541K->20017K(227328K),0.0172573 secs] [Times:user=0.03 sys=0.00,real=0.02 secs]
...
Metaspace： 元数据区GC前后大小的变化，JDK1.8中引入了元数据区（MetaSpace）以替代永久代（PermGen）
xxx secs：指GC花费的时间
...
<img src="https://img-blog.csdnimg.cn/img_convert/4aaef244379d75ba838e10d4178b2960.png" alt="image-20210512195426194" style="zoom:50%;" />

现在 G1 回收器已成为默认回收器好几年了。

我们还看到了引入了两个新的收集器：ZGC（JDK11 出现）和 Shenandoah（Open JDK12）。主打特点：低停顿时间

<img src="https://img-blog.csdnimg.cn/img_convert/00920e4ae2b2c80a8016e6d8f4632545.png" alt="image-20210512195528695" style="zoom:50%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/59b014ca2eeda5332fd40a4a2356f883.png" alt="image-20200714093243028" style="zoom:33%;" />
